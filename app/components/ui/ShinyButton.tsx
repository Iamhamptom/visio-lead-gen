...
export const ShinyButton = ({ text = "shiny-button", className, onClick }: ShinyButtonProps) => {
    return (
        <motion.button
            {...animationProps}
            onClick={onClick}
        ...
        >
            ...
            <span
                ...
                className="absolute inset-0 block h-full w-full bg-[linear-gradient(-75deg,hsl(var(--primary)/10%)_calc(var(--x)+20%),hsl(var(--primary)/50%)_calc(var(--x)+25%),hsl(var(--primary)/10%)_calc(var(--x)+100%))]"
            />
        </motion.button>
    );
};
